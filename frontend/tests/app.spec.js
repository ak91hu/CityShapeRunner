...
test("designer controls are accessible and fit a narrow viewport", async ({ page }) => {
  ...
  await expect(page.getByText("GPS Art Wizzard", { exact: true })).toHaveCount(1);
  await expect(
...
test("quick idea generation sends the prompt and renders a usable routed result", async ({
  ...
  await expect(
    page.locator(".metric").filter({ hasText: "Drawing likeness" }).locator("dd:not(.metric-detail)"),
  ).toHaveText("87%");
  await expect(
    page
      .locator(".metric")
  ...
  await expect(
    page.locator(".route-facts div").filter({ hasText: "Route options shown" }),
  ).toContainText("2");
