import { defineConfig, devices } from "playwright/test";

const PORT = 4173;

export default defineConfig({
  testDir: "./tests",
  fullyParallel: true,
  forbidOnly: Boolean(process.env.CI),
  retries: process.env.CI ? 2 : 0,
  // Leaflet exercises browser rendering and ResizeObserver teardown. A single
  // worker keeps local and CI runs deterministic and resource-efficient.
  workers: 1,
  reporter: process.env.CI ? [["line"], ["html", { open: "never" }]] : "line",
  use: {
    baseURL: `http://127.0.0.1:${PORT}`,
    trace: "on-first-retry",
    screenshot: "only-on-failure",
  },
  projects: [
    {
      name: "chromium",
      use: { ...devices["Desktop Chrome"] },
    },
    {
      name: "mobile-chromium",
      use: { ...devices["Pixel 7"] },
    },
  ],
  webServer: {
    // Launch Vite directly so Playwright owns the actual server process. An
    // intermediate npm process can leave the Windows runner waiting at teardown.
    command: `node ./node_modules/vite/bin/vite.js --host 127.0.0.1 --port ${PORT}`,
    url: `http://127.0.0.1:${PORT}`,
    // A previous preview server can serve stale modules or disappear midway
    // through a run. Always let Playwright own a fresh, isolated test server.
    reuseExistingServer: false,
    timeout: 120_000,
  },
});
