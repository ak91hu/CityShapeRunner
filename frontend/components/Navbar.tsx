    ...
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white">
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
        <Link href="/" className="flex items-center gap-2.5 group">
          <div className="flex h-9 w-9 items-center justify-center rounded-md bg-slate-900 text-white transition-transform group-hover:scale-105">
            <RouteIcon size={20} />
          </div>
          <div className="flex flex-col leading-none">
            <span className="text-base font-bold tracking-tight">CityShapeRunner</span>
            <span className="text-[10px] font-medium text-slate-500">GPS Art Generator</span>
          </div>
        </Link>
          ...
          <div className="flex items-center rounded-md border border-slate-200 bg-white p-0.5">
            <button
              className={`rounded px-2 py-1 text-xs font-bold transition-colors ${
                lang === "hu" ? "bg-slate-900 text-white" : "text-slate-500 hover:text-slate-900"
              }`}
              onClick={() => setLang("hu")}
            >
              HU
