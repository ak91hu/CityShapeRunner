...
class GenerationService:
    def create_job(self, req: GenerationJobCreate, ip: str | None = None) -> tuple[str, str]:
        ...
        # Check global cache for pre-generated candidates
        if not req.force:
            cached_matches = []
            for c in STORE.cached_candidates:
                if STORE.candidate_city.get(c.candidate_id) != req.city_id:
                    continue
                if abs(c.distance_km - req.target_distance_km) / req.target_distance_km > 0.2:
                    continue
                if STORE.candidate_activity.get(c.candidate_id) != req.activity.value:
                    continue
                if req.artwork_ids and c.artwork_id not in req.artwork_ids:
                    continue
                cached_matches.append(c)
            
            if len(cached_matches) >= req.max_suggestions or (req.artwork_ids and len(cached_matches) > 0):
                cached_matches.sort(key=lambda x: x.scores.fit_score, reverse=True)
                job_id = STORE.new_id("job")
                job = JobRecord(id=job_id, request=req, request_hash=h, city_id=req.city_id, status="completed", progress_stage="completed", progress_percent=100)
                job.candidates = cached_matches[:req.max_suggestions]
...
def _activity_for_candidate(candidate_id: str) -> str:
    if candidate_id in STORE.candidate_activity:
        return STORE.candidate_activity[candidate_id]
...
class ExportService:
    def gpx(self, rec: RouteRecord, mode: str) -> tuple[str, str]:
        ...
        fname = gpx_mod.file_name(city_name, rec.artwork_name, rec.distance_km, rec.activity)
        return text, fname
